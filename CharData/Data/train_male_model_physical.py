...
def map_floats_to_integer(input_df, attribute):
    # Create a copy of the input DataFrame to avoid modifying the original DataFrame
    output_df = input_df.copy()

    # Create a mapping of unique heights to integers
    attribute_mapping = {num: idx for idx, num in enumerate(output_df[attribute].unique())}

    # Add a new column with the mapped integers
    output_df[attribute+"_category"] = output_df[attribute].map(attribute_mapping)

    print(attribute+" Mapping:")
    for num, idx in attribute_mapping.items():
        print(f"{num} -> {idx}")
...
def create_models(attribute, train, test):
    training_corpus = train['Answer.q1_Description'].values
    training_labels = train[attribute].values
    test_corpus = test['Answer.q1_Description'].values
...
male_num_of_categories = {"Fat_category": 3, "Muscles_category": 3, "Thin_category": 3, "Head_Offset_category": 3, "Height_category": 3, "HeadSize_category": 3}

# Read each document and clean it.
df["Answer.q1_Description"] = df["Answer.q1_Description"].apply(clean)

# Preview the first 5 lines of the loaded data
print(df.head(5))
df = map_floats_to_integer(df, "Fat")
df = map_floats_to_integer(df, "Muscles")
df = map_floats_to_integer(df, "Thin")
df = map_floats_to_integer(df, "Head_Offset")
df = map_floats_to_integer(df, "Height")
df = map_floats_to_integer(df, "HeadSize")


valid_male_attributes = ["Fat_category","Muscles_category","Thin_category","Head_Offset_category","Height_category","HeadSize_category"]
